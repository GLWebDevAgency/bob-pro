...
| `OPENAI_API_KEY` (secret) | Railway | absente (`env.ts:52`) | **POSÉE le 25/07/2026 — D3 ACTÉ par le fondateur** (clé fournie via `.env` API, posée en production par Claude, `--skip-deploys`) | Prérequis du cap GPT Realtime (OBJECTIFS §4.1.2) ; INERTE tant que `BOB_LIVE_ENABLED=false` posé et `BOB_LIVE_PROVIDER=mistral` — l'activation Bob Live reste un lot atomique distinct | Clé présente = OpenAI devient un provider disponible au catalogue ; surveiller le routage des tâches LLM avant activation |
| `GLM_API_KEY` / `DEEPSEEK_API_KEY` (secrets) | Railway | absentes (`env.ts:14-15`) | **ABSENTES** | Optionnelles ; moins de surface de secret | Une clé inutile posée = surface d'attaque gratuite |
...
| `BOB_LIVE_SPEECH_DELIVERY` | Railway | `audited-signed-url-v1` (enum, `env.ts`) | **défaut = `audited-signed-url-v1`** — intégré au bloc le 25/07 (accord Claude+GPT, décision fondateur) | Chemin de restitution vocale : le RTP natif (`openai-native-webrtc-v1`) est de plus verrouillé à la COMPILATION (`OPENAI_NATIVE_WEBRTC_RUNTIME_READY=false`, refus de boot si posé) | Poser le natif avant certification runtime = refus de boot (fail-closed voulu) |
| `BOB_LIVE_PROOF_KEYRING` (secret) + `BOB_LIVE_GLOBAL_MAX_CONCURRENT_SESSIONS`/`BOB_LIVE_PROVIDER_MAX_CONCURRENT_SESSIONS`/`BOB_LIVE_CAPACITY_CONFIG_VERSION` | Railway | absents (schéma additif du train natif 21-24/07) | **ABSENTS** — le trio capacité est TOUT-OU-RIEN (config partielle = refus de boot) et un keyring résiduel incohérent est boot-fatal même live OFF | Contrat d'activation famille 2 étendu par le train GPT Realtime ; consigné le 25/07, neutralisé dans `RESIDUAL_VARIABLES` du test anti-drift | Provisionner partiellement = refus de boot ; à poser uniquement dans le lot atomique d'activation |
...
| `SENTRY_DSN` (secret) | Railway | absent → canal DORMANT (`env.ts`, `sentry-reporter.ts`) | **POSÉE (20/07) — projet `bob-pro-api`, orga `limame-ghassene`, hôte `*.ingest.de.sentry.io` (région UE vérifiée)** | Crash reporting serveur (§B4). Sans DSN, `@sentry/node` n'est même pas chargé : zéro appel réseau, zéro avertissement. **S'AJOUTE** à `ERROR_REPORTER_WEBHOOK_URL` (composition), ne le remplace pas | DSN hors UE = **refus de boot** (garde `sentryDsnRejectionReason`, fail-closed) : Sentry crée les orgas en région **US par défaut** |
...
| `VOICE_TRACE_ENABLED` | Railway | `false` (`env.ts`) | **défaut = `false`** (DORMANT) | Traçage du comportement vocal du bêta-test (une ligne par TOUR : texte prononcé, intention, outil + paramètres, issue, réponse, latences), **souverain** — table `voice_traces` dans notre PostgreSQL sous RLS FORCE, rétention 30 j purgée, jamais chez un tiers (`telemetry-scrubbing.ts` interdit ces données à Sentry). Sans le flag, aucune ligne n'est écrite : zéro coût, zéro donnée | `true` = stock de transcripts en clair (noms clients, montants, SIRET) pendant 30 j — à n'activer que le temps d'une campagne de test |
| `EXPO_PUBLIC_SENTRY_DSN` / `EXPO_PUBLIC_SENTRY_ENVIRONMENT` | `eas.json` (mobile) | absents → canal DORMANT (`crash-reporter.ts`) | **DSN POSÉ (20/07) dans les DEUX profils — projet `bob-pro-mobile`, région UE ; `_ENVIRONMENT` reste ABSENT** | Crash reporting terrain (plantage profil fiscal indiagnosticable). **Volontairement HORS de la garde `required()` de `app.config.ts`** : un build EAS ne doit jamais dépendre d'un canal d'observabilité | Les déclarer obligatoires casserait TOUS les builds EAS ; un DSN non-UE désactive le canal (jamais de plantage induit) |
...
10. ~~**GPT — accord sur le périmètre C1 révisé** (voix V1 = Voxtral tour-par-tour ON, full-duplex OFF), noté PROGRAMME_V1 l.76.~~ **SOLDÉ le 25/07/2026** : décision fondateur confirmée oralement (contre-signature Claude dans OBJECTIFS_SPECS_DOD_PUBLICATION.md) — cap GPT Realtime avec bascule d'**administration** GPT Realtime ↔ Voxtral tour-par-tour (config runtime, futur dashboard admin — jamais un sélecteur utilisateur) ; duplex Mistral = chantier parallèle hors chemin critique. L'activation reste conditionnée à D3 (clé OpenAI prod).
...
<!-- FLAGS_V1_JSON_START -->
{
  "flags": [
    ...
    { "name": "VOICE_TRACE_ENABLED", "v1Value": "false", "scope": "api", "enforcement": "default" },
    ...
    { "name": "EXPO_PUBLIC_API_URL", "v1Value": "https://bob-pro-api-production.up.railway.app", "scope": "mobile-production", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_SUPABASE_URL", "v1Value": "https://cvdkqjczgqoeshputacl.supabase.co", "scope": "mobile-production", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_API_URL", "v1Value": "https://bob-pro-api-staging.up.railway.app", "scope": "mobile-preview", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_SUPABASE_URL", "v1Value": "https://afywrrzjjuyznewzvpmk.supabase.co", "scope": "mobile-preview", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_TERMS_URL", "v1Value": "https://bob-pro-sign-web.vercel.app/legal/conditions-utilisation", "scope": "mobile-production", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_PRIVACY_URL", "v1Value": "https://bob-pro-sign-web.vercel.app/legal/confidentialite", "scope": "mobile-production", "enforcement": "posed" },
    { "name": "EXPO_PUBLIC_SIGNUP_CONFIRMATION_WEB_URL", "v1Value": "https://bob-pro-sign-web.vercel.app/auth/confirme", "scope": "mobile-production", "enforcement": "posed" },
