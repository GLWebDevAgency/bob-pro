...
| O5 — Voice Trace              | `implemented` dans `9f652f52` — journal V2 réellement câblé du bootstrap à l’ACK/barge-in, contenu chiffré, RLS/append-only, rétention/effacement, consentement mobile et opérateur staging exact-SHA ; 385 tests ciblés, 634 contrats release, 79 contrats schéma, build API et certificat PostgreSQL non-superuser 7/7 verts ; aucune activation production                                                                                                                                                                                                                                                                                                                              | CI du commit fusionné, migration/certificat sur Supabase staging, puis drill OFF→ON→OFF→ON exact-SHA avec canary WebRTC et flag final ON ; dashboard p50/p95 et preuves device ensuite          |
