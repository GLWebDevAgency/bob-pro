...
| O5 — Voice Trace              | `implemented` dans `9f652f52` — journal V2 réellement câblé du bootstrap à l’ACK/barge-in, contenu chiffré, RLS/append-only, rétention/effacement, consentement mobile et opérateur staging exact-SHA ; migrations/certifications Supabase staging et release normale `30714091501` vertes sur `64adf909` ; M2-A V2 réactivé par `30716418193` ; premier drill Trace `30717462924` arrêté avant mutation car le snapshot exigeait à tort le contrat readiness courant sur une production saine encore au profil historique ; aucune activation production                                                                                                                                                                                     | Micro-fix fail-closed du snapshot production historique/courant, CI + merge, puis drill OFF→ON→OFF→ON exact-SHA avec canary WebRTC et flag final ON ; dashboard p50/p95 et preuves device ensuite |
