...
> Dernière mise à jour : 2026-07-30 (amendements A1 → A30, voir § Journal des amendements)
...
| **A27** | 2026-07-30 | **Quatre dettes de rigueur soldées** : (1) la source de comportement était citée par sa **branche** — épinglée sur `expo-glass-tabs@0.1.1`, shasum, integrity et `gitHead` ; (2) le **double glyphe** et l'interdiction d'`onLayout` étaient écrits comme des exigences alors que ce sont des **moyens** — invariant et moyen séparés, alternative recevable sous trois conditions ; (3) les **contrôles statiques** de [11](11-test-strategy.md#tests-statiques) étaient cités comme preuves de sortie alors qu'aucun n'existait dans le dépôt — état réel écrit, `NOT RUN` tant qu'ils ne sont pas exécutables, **et un premier validateur livré** : `scripts/check-mobile-experience-docs.mjs` (16 contrôles verts au 2026-07-30) qui valide le socle **contre le code** et a lui-même trouvé une ancre morte dans [02](02-roadmap.md) ; (4) l'**enveloppe de la retombée** dépendait d'une hauteur de chrome qui s'anime tout en se déclarant « jamais animée » — enveloppe dimensionnée une fois sur l'état étendu, `transform` interdit, mode flouté exclu d'un chrome qui se minimise. Plus l'a11y des **doubles SVG** (glyphes retirés de l'arbre d'accessibilité). | Registre npm `expo-glass-tabs` ; inspection du dépôt le 2026-07-30 ; `apps/mobile/src/components/icons.tsx`. **(source rectifiée A30 · 2026-07-30 ; rédaction A27 supersédée : « aucun `scripts/`, aucun job CI, `package.json` racine »)** Les trois faits étaient faux au moment même où A27 les écrivait : `scripts/` est créé par le commit d'A27 lui-même, `.github/workflows/` portait déjà **huit** workflows, et le `package.json` racine n'est pas le seul (12 manifestes). C'est la famille d'erreur qu'A27 prétendait fermer, survivant dans la rangée d'A27 ; A28 l'avait soldée en [11](11-test-strategy.md) mais pas dans le journal qui l'arbitre. Contrôle `C9` élargi en conséquence. | [02](02-roadmap.md), [04](04-navigation-scroll-surfaces.md), [11](11-test-strategy.md), [16](16-implementation-backlog.md), [17](17-references.md), [19](19-glossary.md), [adr/README](adr/README.md), `scripts/check-mobile-experience-docs.mjs` (nouveau), ce fichier |
| **A28** | 2026-07-30 | **Un seul mécanisme de cible tactile, et il est exécutable sur Android** : `A17` avait raison sur le principe (la retombée `pointerEvents="none"` ne porte aucune cible) mais son mécanisme de remplacement — un `hitSlop` vertical débordant dans la retombée — **n'est jamais dispatché** : la recherche de cible n'entre dans un enfant que si le point est déjà dans les bornes de l'ancêtre, sur Android comme sur iOS ; la pilule livrée n'offre d'ailleurs que `paddingVertical: 8` de budget ; et la prose contredisait la table des régimes en décrivant une pilule Android à 44 que cette table interdit. La cible est désormais tenue par la **hauteur mesurée du `Pressable`**, la pilule se déduit de lui, et cinq mesures d'acceptation remplacent une intention. Trois contradictions soldées au passage : *(rédaction A27, supersédée)* « pas de répertoire `scripts/` » écrit dans le commit qui crée `scripts/` ; deux tableaux **cassés au rendu** par une ligne vide (journal des amendements, registre des risques) ; et « hauteur du chrome » conservé en [10](10-performance-observability.md) là où [04](04-navigation-scroll-surfaces.md) dit « hauteur **ÉTENDUE** ». Plus la portée de « Reduce Transparency sans effet », qui rendait sans objet la règle fail-closed d'`A18`. | Lecture du code livré `packages/ui/src/components/bottom-tab-bar.tsx` (l. 60, 78, 111) ; sémantique de dispatch RN Android (`TouchTargetHelper`) et iOS (`hitTest:`). | [04](04-navigation-scroll-surfaces.md), [08](08-accessibility-adaptive-design.md), [10](10-performance-observability.md), [11](11-test-strategy.md), [14](14-risk-register.md), [19](19-glossary.md), [`RN_EXPO_GUIDE.md`](../../design_handoff_bob_pro/RN_EXPO_GUIDE.md), `scripts/check-mobile-experience-docs.mjs`, ce fichier |
| **A29** | 2026-07-30 | **La couture du port de flou, écrite de bout en bout** : le contrat de props d'`A20` était exact mais s'arrêtait à la frontière de paquet — le kit restait indevinable. Sont désormais posés : le **type TypeScript** du port (`BlurLayerSpec`, `RenderBlurLayer`, arguments nommés et retour `ReactElement` ou `null`), la règle **tout ou rien**, le **propriétaire** du `BlurTargetView` (`apps/mobile`, au shell d'écran, en `StyleSheet.absoluteFill`), le fait que la `ref` **ne traverse jamais** la frontière (le port est une closure), le calcul du profil de hauteurs par `@bob/ui`, et l'**englobement garanti par construction** (deux rectangles du même parent). Deux contraintes officielles manquaient : l'englobement des couches, et surtout le flou qui **ne se rafraîchit pas** au-dessus d'un contenu dynamique — `@shopify/flash-list` `^2.0.2` est déjà déclaré dans `apps/mobile/package.json`. Cinquième coupure du repli opaque unique, ordre de montage imposé, contrôle statique et preuve filmée de levée ; `R45` ajouté. | Documentation Expo SDK 57 `expo-blur` (limitations officielles : englobement dans un `BlurTargetView` unique, non-rafraîchissement au-dessus d'un contenu dynamique) ; `apps/mobile/package.json` lu le 2026-07-30. | [04](04-navigation-scroll-surfaces.md), [10](10-performance-observability.md), [11](11-test-strategy.md), [14](14-risk-register.md), [19](19-glossary.md), ce fichier |
| **A30** | 2026-07-30 | **L'ordre de peinture, et ce que le squelette cachait** : `A29` prescrivait deux choses inconciliables. Son sous-§ 2 monte le `BlurTargetView` « dans le parent commun qui contient le contenu **et** le chrome » ; son sous-§ 5 impose que `ProgressiveBlurBob` soit « le **dernier** enfant du shell, sans exception » — repris en contrôle statique, en critère d'acceptation et en preuve de sortie de `WP-0307`. En React Native le frère déclaré **en dernier est peint au-dessus** : un chrome monté dans ce parent passait **sous** la retombée, dont le voile est opaque **dès 60 %** vers le bord ancré, c'est-à-dire exactement là où vit la pilule. Le squelette **omettait le chrome**, ce qui rendait le conflit invisible à la lecture — un squelette qui cache la pièce en litige n'est pas un contrat. Tranché : **`CONTENU → RETOMBÉE → CHROME`**, squelette complété, autorité de l'ordre écrite (déclaration seule ; ni `zIndex`, ni `elevation`, ni token d'ombre sur la retombée, parce que sur Android l'`elevation` trie l'ordre de dessin et ferait diverger les deux OS), englobement revérifié après le changement d'ordre. Cinq contradictions documentaires soldées au passage : la source d'A27 (« aucun `scripts/`, aucun job CI ») fausse au dépôt ; les bornes « `A1` → `A27` » de ce fichier contre « A1 → A29 » de son propre en-tête ; `A18` absent de l'index de [04](04-navigation-scroll-surfaces.md) ; la mesure n° 2 des cibles tactiles, fausse d'un pixel de bordure dans **les deux** états ; et la ligne « Coupures obligatoires » de [10](10-performance-observability.md), qui n'en listait que quatre sur cinq. Trois contrôles ajoutés au validateur (`C11` bornes, `C12` index d'amendements, `C9` élargi). | Sémantique de rendu React Native (ordre de déclaration ; Android trie le dessin d'un `ViewGroup` par `Z = elevation + translationZ`) ; **code livré** `packages/ui/src/components/bottom-tab-bar.tsx` l. 110 et 118, qui déclare le `LinearGradient` **avant** la pilule ; `controls.cardBorder` / `borderWidth: 1` l. 58-59 ; inspection du dépôt le 2026-07-30 (`scripts/` présent, `.github/workflows/` = 8 workflows). | [00](00-audit-baseline.md), [01](01-experience-vision.md), [02](02-roadmap.md), [03](03-motion-interaction-system.md), [04](04-navigation-scroll-surfaces.md), [09](09-technical-architecture.md), [10](10-performance-observability.md), [11](11-test-strategy.md), [12](12-definition-of-done.md), [15](15-traceability-matrix.md), [16](16-implementation-backlog.md), [17](17-references.md), [19](19-glossary.md), [adr/README](adr/README.md), [UX-ADR-002](adr/UX-ADR-002-navigation-surfaces.md), [UX-ADR-004](adr/UX-ADR-004-adaptive-appearance.md), `scripts/check-mobile-experience-docs.mjs`, ce fichier |
...
**Défauts de code signalés, non corrigés (2026-07-30).** **Quatre** constats faits en écrivant
`A18`, `A23`, `A26` et `A28` relèvent du **code**, pas du document, et sont hors de ce lot
documentaire. Ils sont consignés ici pour ne pas se perdre :

1. le **fail-OPEN** de `packages/ui/src/hooks/use-reduce-motion.ts` (`A18`) ;
2. le couple texte/fond à **4,39:1** de `apps/mobile/app/equipements/[chantierId].tsx` l. 274
   (`A23`) ;
3. l'`accessibilityState.disabled` qui reste `false` pendant `loading` dans
   `packages/ui/src/components/button.tsx` (`A26`) ;
4. **(ajouté `A28`)** la **cible tactile à 44 dp sur Android** de la barre livrée :
   `packages/ui/src/components/bottom-tab-bar.tsx` l. 78 pose `minHeight: 44` sans distinction de
   plateforme, et le contenu de l'onglet mesure ~39 pt — la cible réelle vaut donc 44 dp sur
   Android, sous le plancher de **48 dp** que
   [04 § Cibles tactiles et Dynamic Type](04-navigation-scroll-surfaces.md#cibles-tactiles-et-dynamic-type)
   et [08 § Cibles tactiles](08-accessibility-adaptive-design.md#cibles-tactiles) posent comme
   minimum produit.

Les points 1, 2 et 4 sont des écarts **d'accessibilité**. Aucun n'est réparable par une phrase, et
aucun n'est corrigé ici : le point 4 tomberait de surcroît sous la **borne de livraison n° 1** de
[04](04-navigation-scroll-surfaces.md#bornes-de-livraison) — « la `BottomTabBar` existante n'est ni
restylée ni supprimée ».

**Portée hors dossier.** `A7`, `A10`, `A11`, `A12`, `A15`, `A16` — et, le 2026-07-30, `A17`, `A24`,
`A26` et `A28` — amendent des documents canoniques **hors** `docs/mobile-experience` : trois fichiers du
...
daté qui nomme sa source et sa portée ; le tableau ci-dessus fait foi sur **quels** documents un
amendement touche, l'encadré de chaque fichier sur **où** il l'amende — une énumération recopiée
devient fausse au premier amendement suivant. **(corrigé A30 · 2026-07-30 : cette phrase portait
les bornes « `A1` → `A27` » alors que l'en-tête du même fichier disait « A1 → A29 » — elle se
démontrait elle-même. Les bornes sont retirées d'ici, et celles qui subsistent ailleurs dans le
socle sont désormais tenues par le contrôle `C11` du validateur.)** La
