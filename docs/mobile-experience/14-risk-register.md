...
| R40 **(ajouté A3 · 2026-07-29)** | Portage de la tab bar : première entrée de Reanimated et Gesture Handler dans le produit | Élevé | Élevé | Le composant est fluide en dev et saccade en release ; conflits de geste avec le scroll ; régression du geste Retour. | `UX-ADR-001` accepté avant le portage ; `PERF-13` obligatoire ; seuils de geste repris tels quels de la référence ; flag `mobile_tabs_experiment_v1` OFF par défaut ; `BottomTabBar` actuelle conservée comme fallback. | Mobile tech lead |
