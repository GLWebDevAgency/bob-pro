...
Dernière mise à jour : **2026-07-30** (2026-07-29 — A1/A2/A3 : entrées matière, retombée et tab
bar ; A8 : teinte pilotée par le highlight ; A12 : durée du fade-through ; A13 : affirmation
« verre système » ; A14 : Reduce Transparency. **2026-07-30** — A17/A18/A22/A23 : cible tactile,
préférence inconnue, scrub, `BobSurface` ; **A28** : ce qu'un `hitSlop` peut et ne peut pas ;
**A29** : couture du port de flou. **La ligne de date était restée au 2026-07-29 alors que le
glossaire avait été amendé le 2026-07-30 — corrigé A28.**)
...
| Cible tactile **(ajouté A17 · 2026-07-30 ; précisé A28)** | Surface qui **reçoit réellement la touche** : un `Pressable`/`Touchable`, plus son `hitSlop` **s'il reste dans les bornes du parent**. Minimum produit 44 × 44 pt (iOS) / 48 × 48 dp (Android). Elle est **indépendante du visuel** : un glyphe plus petit est le cas nominal. Deux vérités négatives : une vue `pointerEvents="none"` — dégradé, scrim, retombée de bord, halo — ne reçoit aucune touche et ne peut donc jamais tenir une cible, même si ses pixels coïncident ; et un `hitSlop` qui **déborde un ancêtre** n'est jamais dispatché, sur aucun des deux OS — il n'agrandit donc rien au-delà du padding de son parent. Une cible se **mesure**, elle ne se lit pas dans un style. |
...
| `ProgressiveBlurBob` | **(ajouté A2 · 2026-07-29)** Composant de retombée de bord de Bob. Mode nominal : dégradé teinté, zéro échantillon de flou, un seul draw call. Mode flouté optionnel : empilement de couches **frères** de hauteurs décroissantes, chacune teintée Bob, derrière un port injecté `renderBlurLayer` ; repli opaque unique si le port est absent, en Reduce Transparency, sur Android dégradé, hors budget **ou au-dessus d'une liste virtualisée (ajouté A29 · 2026-07-30)**. **(précisé A29)** Le `BlurTargetView` que les couches échantillonnent appartient à `apps/mobile` et se monte au shell d'écran ; la `ref` ne traverse jamais la frontière de paquet — le port est une closure. **(précisé A30 · 2026-07-30)** Dans ce shell, l'ordre de déclaration est **`CONTENU → RETOMBÉE → CHROME`** : la retombée est peinte **sur** le contenu et **sous** le chrome, parce qu'elle existe pour dissoudre le contenu avant qu'il n'atteigne le chrome — jamais pour le recouvrir. Ni `zIndex`, ni `elevation`, ni token d'ombre sur la retombée. |
...
| Minimize-on-scroll | **(ajouté A3 · 2026-07-29 ; précisé A17/A19 · 2026-07-30)** Repli de la tab bar au scroll vers le bas : la pilule rétrécit dans les deux dimensions, les labels s'effacent, **tous les onglets restent visibles et atteignables**. Seul le **visuel** rétrécit : la cible tactile reste ≥ 44 pt (iOS) / 48 dp (Android), tenue par la **hauteur du `Pressable` lui-même** — la pilule se déduit de lui, jamais l'inverse, et aucun `hitSlop` n'intervient **(mécanisme tranché A28 · 2026-07-30)**. Sur Android la pilule repliée mesure donc 48 dp, jamais 44. Les hauteurs sont **calculées** (plancher + contenu mesuré), jamais figées en points. Elle se ré-étend au scroll vers le haut, sous 24 px du sommet, et à toute interaction avec la barre. Ne pas confondre avec l'effondrement sur une seule icône d'`UITabBar`. |
