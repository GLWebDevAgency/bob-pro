...
- ~~**01/09/2026** : décret CIBS — PR-20 obligatoire avant cette date (risque de mention légale fausse sinon).~~ *[amendé 28/07/2026 — jalon FAUX, cf. amendement en tête]* → **01/01/2027** : entrée en vigueur du transfert de la TVA dans le CIBS (ord. n° 2026-671 du 27/07/2026) — puis **30/06/2028** : fin de la tolérance des anciennes références au CGI, seule date après laquelle la mention imprimée aujourd'hui deviendrait non conforme. Aucune action calendaire à planifier ici : les deux échéances sont **armées dans le code** (`packages/core/src/domain/services/veille-mentions-legales.ts`), sous un régime à **deux étages** *[amendé 28/07/2026 — la version précédente disait « la CI échoue 90 j avant la première et 180 j avant la seconde » : c'était vrai, et c'était le défaut. Une CI rouge pendant 90 jours sur des PR sans rapport se termine par la désactivation du test — et une alarme qu'on désactive ne protège plus personne]* : **préavis (90 j / 180 j) VISIBLE mais NON bloquant** — annotation GitHub + résumé de run sur chaque PR et chaque push de main, plus le signal au démarrage de l'API —, puis **BLOCAGE de la CI** le jour où l'échéance est atteinte (01/01/2027, puis 30/06/2028). Seul moyen d'apaiser un préavis : mettre à jour `verifieLe` dans le registre — geste daté, relu en diff, qui ré-arme l'alarme seul au bout de 30 jours. Aucune variable d'environnement, aucun skip.
