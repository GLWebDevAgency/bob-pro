...
- ✅ **Plancher de sécurité INVIOLABLE** (confirmation toujours requise, même en `auto`) : **envoi vers un tiers** (`outbound` : devis/facture/relance) + **irréversible légal/fiscal** (`safetyFloor` : émettre une facture) + **purge de document** + **posting d'un paiement** (`encaisser`/marquer payé, `safetyFloor:true`). **Réconcilié Claude↔Codex (2026-07)** : Claude penchait « réversible → direct », Codex a argumenté (mieux) que le posting modifie les livres (CA/TVA/statut client/relances/rapprochement) → **fausse réalité comptable** même réversible ; Claude concède. Toujours confirmer, via un OK voix/tap rapide (`confirmByVoice`). **Nuance prepare-vs-post** (Codex) : un futur `draft_payment`/`prepare_reconciliation` (préparation sans posting) pourra tourner en `auto` ; seul le *posting* effectif est au plancher. Mécanisme : `Tool.safetyFloor` + `isSafetyFloor` + `requiresConfirmation` (packages/ai).
