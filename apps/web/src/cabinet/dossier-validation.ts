...
/**
 * Validation stricte du contrat wire Cabinet. Ce module ne dépend d'aucune API navigateur,
 * d'aucun stockage et n'applique aucune valeur de secours.
 */
...
export interface CabinetDossierValidationError {
  readonly code: 'invalid_response';
  readonly message: string;
  readonly path: string;
}

export type CabinetDossierValidationResult =
  | { readonly ok: true; readonly value: CabinetDossier }
  | { readonly ok: false; readonly error: CabinetDossierValidationError };

export function validateCabinetDossierValue(
  value: unknown,
): CabinetDossierValidationResult {
  const issue = validateDossier(value, '$');
  if (issue) {
    return {
      ok: false,
      error: { code: 'invalid_response', message: issue.message, path: issue.path },
    };
  }
  return { ok: true, value: cloneDossier(value as CabinetDossier) };
}
