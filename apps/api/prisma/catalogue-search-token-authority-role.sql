...
-- Supabase refuse les mutations d'adhésion explicites visant son rôle postgres. PostgreSQL 16+
-- crée donc uniquement la capacité SET du déployeur au moment où celui-ci crée l'autorité.
SET createrole_self_grant = 'set';

SELECT pg_catalog.format(
  'CREATE ROLE %I NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS',
  'bob_catalogue_search_token_sync'
)
WHERE NOT EXISTS (
  SELECT 1
...
DO $catalogue_search_token_authority_membership$
DECLARE
  deployer_oid OID;
...
BEGIN
  ...
  SELECT role.oid
    ...
    FROM pg_catalog.pg_roles AS role
   WHERE role.rolname = 'bob_catalogue_search_token_sync';
  ...
  IF NOT pg_catalog.pg_has_role(current_user, authority_oid, 'SET')
    ...
    RAISE EXCEPTION
      'bob_catalogue_search_token_sync requires implicit SET membership from this deployer';
  END IF;
END;
$catalogue_search_token_authority_membership$;

-- Le rôle NOLOGIN ne peut hériter d'aucune autre autorité. Une adhésion Supabase impossible à
-- révoquer proprement fait échouer le certificat de forme ci-dessous au lieu d'être ignorée.
SELECT pg_catalog.format(
 ...
 WHERE membership.member = 'bob_catalogue_search_token_sync'::pg_catalog.regrole
...
DO $catalogue_search_token_authority_shape$
DECLARE
...
BEGIN
  ...
  IF authority.rolcanlogin
    ...
    RAISE EXCEPTION 'Catalogue search token authority role privilege drift';
  END IF;
  IF EXISTS (
    SELECT 1
      FROM pg_catalog.pg_auth_members AS membership
    ...
    RAISE EXCEPTION 'Catalogue search token authority has an unexpected membership';
  END IF;
END;
$catalogue_search_token_authority_shape$;
