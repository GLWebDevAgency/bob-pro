...
DO $bob_m2a2_cutover_release_flags_owner$
DECLARE
  owner_oid OID;
  owner_name TEXT;
BEGIN
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    IF owner_name IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_RELEASE_FLAGS_OWNER_UNAVAILABLE';
    END IF;
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_RELEASE_FLAGS_OWNER_NOT_ASSUMED';
  END IF;
END;
$bob_m2a2_cutover_release_flags_owner$;

ALTER TABLE public.release_flags NO FORCE ROW LEVEL SECURITY;

DO $bob_m2a2_cutover_release_flag_exact$
BEGIN
  IF (
  ...
  OR EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_FLAG_NOT_EXACTLY_OFF';
  END IF;
END;
$bob_m2a2_cutover_release_flag_exact$;

ALTER TABLE public.release_flags ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.release_flags FORCE ROW LEVEL SECURITY;
RESET ROLE;

DO $bob_m2a2_cutover_missions_owner$
DECLARE
  owner_oid OID;
  owner_name TEXT;
BEGIN
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    IF owner_name IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_MISSIONS_OWNER_UNAVAILABLE';
    END IF;
    EXECUTE pg_catalog.format('SET LOCAL ROLE %I', owner_name);
  END IF;
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_MISSIONS_OWNER_NOT_ASSUMED';
  END IF;
END;
$bob_m2a2_cutover_missions_owner$;
...
DO $bob_m2a2_cutover_events_owner$
DECLARE
  owner_oid OID;
  owner_name TEXT;
BEGIN
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    IF owner_name IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_EVENTS_OWNER_UNAVAILABLE';
    END IF;
    EXECUTE pg_catalog.format('SET LOCAL ROLE %I', owner_name);
  END IF;
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_EVENTS_OWNER_NOT_ASSUMED';
  END IF;
END;
$bob_m2a2_cutover_events_owner$;
...
DO $bob_m2a2_cutover_line_work_owner$
DECLARE
  owner_oid OID;
  owner_name TEXT;
BEGIN
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    IF owner_name IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_LINE_WORK_OWNER_UNAVAILABLE';
    END IF;
    EXECUTE pg_catalog.format('SET LOCAL ROLE %I', owner_name);
  END IF;
  ...
  IF current_user::pg_catalog.regrole <> owner_oid THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'AGENT_MISSION_M2A2_CUTOVER_LINE_WORK_OWNER_NOT_ASSUMED';
  END IF;
END;
$bob_m2a2_cutover_line_work_owner$;
