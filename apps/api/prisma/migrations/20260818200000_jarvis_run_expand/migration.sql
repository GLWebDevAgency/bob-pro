...
-- (même patron que 20260729110000_agent_mission_global_foreground_expand).
DO $bob_jarvis_u1a_owner$
DECLARE
...
BEGIN
  ...
  IF current_user::pg_catalog.regrole <> schema_owner_oid THEN
    IF schema_owner_name IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'JARVIS_U1A_SCHEMA_OWNER_UNAVAILABLE';
    END IF;
  ...
  IF current_user::pg_catalog.regrole <> schema_owner_oid THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'JARVIS_U1A_SCHEMA_OWNER_NOT_ASSUMED';
  END IF;
END;
$bob_jarvis_u1a_owner$;
