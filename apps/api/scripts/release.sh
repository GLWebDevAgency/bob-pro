...
# deploiement des migrations (comme tous les ensure_*) ; la bascule SECURITY DEFINER vient APRES.
...
SELECT format(
  'CREATE ROLE %I NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS',
  'bob_jarvis_payload_retention_directory'
)
WHERE NOT EXISTS (
  SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'bob_jarvis_payload_retention_directory'
) \gexec
...
BEGIN
  ...
  SELECT role.oid
    ...
    FROM pg_catalog.pg_roles AS role
   WHERE role.rolname = 'bob_jarvis_payload_retention_directory';
  ...
  IF NOT pg_catalog.pg_has_role(current_user, authority_oid, 'SET')
    ...
    RAISE EXCEPTION
      'bob_jarvis_payload_retention_directory is not available through implicit SET membership; create it as this deployer with createrole_self_grant=set before retrying';
  END IF;
END;
$$;
...
ALTER ROLE bob_jarvis_payload_retention_directory
  NOLOGIN NOCREATEDB NOCREATEROLE NOINHERIT;
...
BEGIN
  SELECT * INTO STRICT authority
    FROM pg_catalog.pg_roles WHERE rolname = 'bob_jarvis_payload_retention_directory';
  IF authority.rolcanlogin OR authority.rolsuper OR authority.rolcreatedb
     OR authority.rolcreaterole OR authority.rolinherit OR authority.rolreplication
     OR authority.rolbypassrls THEN
...
SELECT format(
  'REVOKE %I FROM %I CASCADE', parent.rolname, 'bob_jarvis_payload_retention_directory'
)
  FROM pg_catalog.pg_auth_members AS membership
  JOIN pg_catalog.pg_roles AS parent ON parent.oid = membership.roleid
 WHERE membership.member = 'bob_jarvis_payload_retention_directory'::regrole
   AND parent.rolname <> 'postgres'
\gexec
SELECT format(
  'REVOKE %I FROM %I CASCADE', 'bob_jarvis_payload_retention_directory', member.rolname
)
  FROM pg_catalog.pg_auth_members AS membership
  JOIN pg_catalog.pg_roles AS member ON member.oid = membership.member
 WHERE membership.roleid = 'bob_jarvis_payload_retention_directory'::regrole
   AND member.rolname NOT IN (current_user, 'postgres')
\gexec
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'bob_jarvis_payload_retention_directory has an unexpected member; membership remediation must be performed outside this release without targeting postgres';
  END IF;
END;
$$;
SQL
     ...
     WHERE function.oid =
       ...
       AND function.proowner NOT IN (
         ...
         (SELECT relation.relowner FROM pg_catalog.pg_class AS relation
           WHERE relation.oid = 'public.jarvis_proposal_payloads'::regclass),
         'bob_jarvis_payload_retention_directory'::regrole
       )
  ) THEN
...
SELECT format(
  'ALTER FUNCTION %s OWNER TO bob_jarvis_payload_retention_directory',
  function.oid::regprocedure
)
  FROM pg_catalog.pg_proc AS function
 WHERE function.oid =
...
SELECT format(
  'REVOKE ALL PRIVILEGES ON FUNCTION public.list_jarvis_payload_retention_owners_v1(TEXT, INTEGER) FROM %I',
  role.rolname
)
  FROM pg_catalog.pg_roles AS role
...
\gexec
SELECT format(
  'GRANT EXECUTE ON FUNCTION public.list_jarvis_payload_retention_owners_v1(TEXT, INTEGER) TO %I',
  :'app_role'
)
 WHERE :'app_role' <> ''
\gexec
...
SELECT DISTINCT format(
  'REVOKE ALL PRIVILEGES (%I) ON TABLE public.jarvis_proposal_payloads FROM bob_jarvis_payload_retention_directory CASCADE',
  attribute.attname
)
  FROM pg_catalog.pg_attribute AS attribute
 CROSS JOIN LATERAL pg_catalog.aclexplode(attribute.attacl) AS privilege
 WHERE attribute.attrelid = 'public.jarvis_proposal_payloads'::regclass
   AND attribute.attnum > 0 AND NOT attribute.attisdropped
   AND privilege.grantee = 'bob_jarvis_payload_retention_directory'::regrole
\gexec
...
SELECT format('REVOKE %I FROM %I CASCADE', 'bob_jarvis_payload_retention_directory', :'app_role')
 WHERE :'app_role' <> ''
   AND EXISTS (
     SELECT 1
       FROM pg_catalog.pg_auth_members AS membership
