...
test('la migration ferme immédiatement tables et fonctions à la Data API Supabase', () => {
  ...
  assert.match(expand, /ALTER TABLE public\.document_archive_render_snapshots FORCE ROW LEVEL SECURITY/u);
  assert.match(expand, /ALTER TABLE public\.document_archive_artifact_intents FORCE ROW LEVEL SECURITY/u);
});
...
test('le writer V3 vérifie le seal avant l’écriture et le LIST ne l’ouvre pas', () => {
  ...
  assert.doesNotMatch(repository.slice(listDueStart, claimStart), /openDocumentArchiveRenderSnapshot/u);
...
test('le cutover est lié aux migrations, drainé, monotone et retire N-1 atomiquement', () => {
  ...
  assert.doesNotMatch(
    activation,
    /GRANT (?:SELECT|UPDATE) ON TABLE public\._prisma_migrations/u,
  );
...
test('release et CI maintiennent V2 pendant expand puis activent V3 avant postdeploy', () => {
  const grantV3 = release.indexOf('GRANT EXECUTE ON FUNCTION public.document_archive_job_enqueue_v3');
  ...
  const railwaySnapshotV2 = releaseActivation.indexOf(
    'activate-document-archive-snapshot-v2.sh',
  );
  const railwaySettlementV2 = releaseActivation.indexOf(
