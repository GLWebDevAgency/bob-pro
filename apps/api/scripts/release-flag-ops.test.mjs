...
test('bloque toute activation live sans couverture explicite du worker', () => {
  const base = {
    ...
    subjectId: 'cabinet-pilot',
    actor: 'ops',
    reason: 'Activation pilote',
  };
  assert.throws(
    () =>
      validateCabinetPilotActivation(base, {
  ...
  assert.throws(
    () => validateCabinetPilotActivation({ ...base, operation: 'set-global' }, {}),
    /globally enabled/,
  );
});
