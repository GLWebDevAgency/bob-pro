...
test('exige une identité interne et des traces de gouvernance explicites', () => {
  ...
  assert.throws(
    () => parseM1BStagingFlagEnvironment(environment({
...
test('parse un état DB unique et refuse les états contradictoires', () => {
  assert.deepEqual(
    decodeM1BStagingFlagState(JSON.stringify(state())),
    state(),
  );
  assert.throws(
    () => decodeM1BStagingFlagState(JSON.stringify(state({
  ...
  assert.throws(
    () => decodeM1BStagingFlagState('null'),
...
test('préflight exige global OFF, kill switch clair, cible absente et aucun autre pilote', () => {
  ...
  assert.throws(
    () => assertM1BStagingFlagPreflight(state({ enabled: true })),
    /global.*OFF/u,
  );
  assert.throws(
    () => assertM1BStagingFlagPreflight(state({ killSwitch: true })),
    /kill switch/u,
  );
  assert.throws(
    () => assertM1BStagingFlagPreflight(state({
      subjectCount: 1,
...
test('activation utilise le CAS parent et relit exactement le seul override user', () => {
  ...
  assert.equal(
    mutations[0].dependencies.directUrl,
