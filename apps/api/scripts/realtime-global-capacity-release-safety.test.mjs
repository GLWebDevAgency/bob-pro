...
const [
  ...
  readFile(new URL(
    'prisma/migrations/20260722040000_realtime_global_capacity/migration.sql', root,
  ), 'utf8'),
...
test('le plafond physique est transactionnel, N-1 compatible et la suppression société est fenced', () => {
  assert.match(migration, /LOCK TABLE public\.realtime_session_leases IN SHARE ROW EXCLUSIVE MODE/u);
  assert.match(migration, /INSERT INTO public\.realtime_global_capacity[\s\S]*count\(\*\)::INTEGER/u);
  ...
  assert.match(migration, /mode = 'active'[\s\S]*"usedSessions" \+ changed_rows <= "globalMaxSessions"/u);
...
test('la configuration live est tout-ou-rien et possède une version monotone', () => {
  ...
  assert.match(envSource, /globalCapacity\.globalMaxSessions > globalCapacity\.providerMaxSessions/u);
  assert.match(envSource, /globalCapacity\.globalMaxSessions > env\.BOB_LIVE_GATEWAY_MAX_CONNECTIONS/u);
...
test('la release ferme d’abord, certifie en lecture seule, puis active en dernier', () => {
  ...
  const forcedClosed = release.indexOf('BOB_LIVE_ENABLED=false OPENAI_REALTIME_ENABLED=false', provision);
...
test('la CI comporte une autorité active éphémère puis la certification N/N+1', () => {
  assert.match(ci, /BOB_LIVE_GLOBAL_MAX_CONCURRENT_SESSIONS/u);
  assert.match(ci, /BOB_LIVE_PROVIDER_MAX_CONCURRENT_SESSIONS/u);
  assert.match(ci, /BOB_LIVE_CAPACITY_CONFIG_VERSION/u);
  assert.match(ci, /RUN_POSTGRES_REALTIME_CAPACITY_CERT/u);
  assert.match(ci, /realtime-capacity\.postgres\.test\.ts/u);
});
