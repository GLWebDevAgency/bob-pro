...
describe.skipIf(!RUN_POSTGRES_CERT)(
  'Notification outbox — certification PostgreSQL/RLS réelle',
  () => {
    ...
    it('certifie migration, contrainte, index, READ COMMITTED et rôle FORCE RLS', async () => {
      const [role] = await workers[0]!.$queryRaw<
        Array<{
          userName: string;
          rolsuper: boolean;
    ...
    `;
      ...
      await expect(
        workers[0]!.withTenant(companyId, (tx) =>
    ...
    it('upgrade réel : les jobs legacy ambigus sont backfillés puis mis en quarantaine', async () => {
      ...
      `,
        ...
        const rolledBackShape = execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            '-X',
        ...
        execFileSync(
          'psql',
          [upgradeUrl.toString(), '-X', '-v', 'ON_ERROR_STOP=1', '-f', outboxMigrationPath],
          { stdio: 'pipe' },
        );
        execFileSync(
          'psql',
      ...
      `,
        ...
        execFileSync(
          'psql',
          [upgradeUrl.toString(), '-X', '-v', 'ON_ERROR_STOP=1', '-c', HISTORICAL_OUTBOX_RLS],
          { stdio: 'pipe' },
        );

        // Deux CHECK valides portant les bons noms mais une définition permissive ne doivent
        // jamais faire passer le cutover. Le refus intervient avant le retrait du trigger/attribut
        // expand : l'opérateur ne transforme pas une dérive homonyme en état terminal partiel.
        execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'ALTER TABLE public.notification_jobs ADD CONSTRAINT notification_jobs_payload_shape CHECK (true), ADD CONSTRAINT notification_jobs_lease_shape CHECK (true);',
          ],
          { stdio: 'pipe' },
        );
        expect(() =>
        ...
        const preservedExpandShape = execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            '-X',
        ...
        expect(preservedExpandShape).toBe('2');
        execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'ALTER TABLE public.notification_jobs DROP CONSTRAINT notification_jobs_payload_shape, DROP CONSTRAINT notification_jobs_lease_shape;',
          ],
          { stdio: 'pipe' },
        );
        ...
        // Un index public valide mais de mauvaise définition ne doit jamais être accepté par son
        // seul nom. L'opérateur terminal refuse avant toute mutation supplémentaire.
        execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'DROP INDEX public."notification_jobs_due_deliverable_idx"; CREATE INDEX "notification_jobs_due_deliverable_idx" ON public.notification_jobs ("companyId");',
          ],
          { stdio: 'pipe' },
        );
        expect(() =>
        ...
        execFileSync(
          ...
          [
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'DROP INDEX public."notification_jobs_due_deliverable_idx"; CREATE INDEX "notification_jobs_due_deliverable_idx" ON public.notification_jobs ("companyId", "nextAttemptAt", "createdAt") WHERE status IN (\'pending\', \'failed\') AND payload IS NOT NULL;',
          ],
          { stdio: 'pipe' },
        );

        // Un homonyme valide dans un autre schéma ne remplace jamais l'index de la table publique.
        execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'CREATE SCHEMA outbox_shadow; CREATE TABLE outbox_shadow.jobs ("companyId" TEXT, "createdAt" TIMESTAMP); CREATE INDEX "notification_jobs_recent_idx" ON outbox_shadow.jobs ("companyId", "createdAt" DESC); DROP INDEX public."notification_jobs_recent_idx";',
          ],
          { stdio: 'pipe' },
        );
        expect(() =>
        ...
        execFileSync(
          ...
          [
            ...
            'ON_ERROR_STOP=1',
            '-c',
            'CREATE INDEX "notification_jobs_recent_idx" ON public.notification_jobs ("companyId", "createdAt" DESC); DROP SCHEMA outbox_shadow CASCADE;',
          ],
          { stdio: 'pipe' },
        );
        ...
        const [legacy, invalid, safe, cutover] = await Promise.all([
          ...
          upgrade.notificationJob.findUniqueOrThrow({
        ...
        expect(cutover.payload).toEqual(
      ...
      `,
        ...
        execFileSync(
          'sh',
          [resolve(process.cwd(), 'scripts/deactivate-notification-outbox-v2.sh')],
          {
            env: { ...process.env, DIRECT_URL: upgradeUrl.toString() },
            stdio: 'pipe',
          },
      ...
      `,
        ...
        expect(nMinusOneSpooled.payload).toEqual(
          expect.objectContaining({
        ...
        expect(stillQuarantined.lastError).toContain(
          'manual-review:rollback-provider-outcome-uncertain',
        );
        ...
        const rejectedMixedShape = execFileSync(
          'psql',
          [
            upgradeUrl.toString(),
            '-X',
    ...
    it('l’adapter garde le payload immuable et utilise l’horloge DB pour le lease/fence', async () => {
      ...
      const enqueued = await workers[0]!.withTenant(companyId, () =>
        repository.enqueue({
          id,
          companyId,
          kind: 'invoice-relance',
      ...
      await expect(
        workers[0]!.withTenant(companyId, () =>
          repository.enqueue({
            id: conflictingId,
            companyId,
            kind: 'invoice-relance',
      ...
      const claimed = await workers[0]!.withTenant(companyId, () =>
      ...
      expect(Date.parse(claimed.job.nextAttemptAt)).toBeLessThan(
        Date.parse('2090-01-01T00:00:00.000Z'),
      );
      await expect(
        workers[0]!.withTenant(companyId, () =>
          repository.authorizeDeliveryAttempt(
            id,
            companyId,
            leaseToken,
    ...
    it('snapshot non-lu et lecture batch restent atomiques sous horloge DB et concurrence réelle', async () => {
      ...
      try {
        ...
        const stored = await admin.notificationJob.findUniqueOrThrow({
          where: { id: concurrentId },
        });
        expect(stored.readAt?.toISOString()).toBe(observed[0]);
      } finally {
        await admin.notificationJob
          .deleteMany({ where: { companyId: readCompanyId } })
          .catch(() => undefined);
