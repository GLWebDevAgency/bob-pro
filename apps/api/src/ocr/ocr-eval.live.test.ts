...
import { OCR_GOLDEN_CASES, runOcrEval, type OcrEvalOutcome } from '@bob/ai/testing';
...
describe.skipIf(!LIVE)('éval OCR live — Mistral (golden set annoté)', () => {
  it('précision par champ ≥ seuils (TTC 0.8 · fournisseur 0.8 · rejet devise 1.0 · pass 0.6)', async () => {
    ...
    for (const result of report.results)
      console.info(`${result.pass ? '✓' : '✗'} ${result.name}`, result.scores);
