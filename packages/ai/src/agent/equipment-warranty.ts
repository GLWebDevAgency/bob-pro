 ...
 * JJ/MM/AAAA, AAAA-MM-JJ, « jusqu'au 12 mars 2027 » (jour dit) et « mars 2027 » (fin de mois).
...
const SPOKEN_DAY = new RegExp(
  `garanti\\w*[^0-9]{0,24}\\b(\\d{1,2})(?:er)?\\s+(${MONTH_ALTERNATION})\\s+(\\d{4})\\b`,
);

/** « garantie jusqu'en mars 2027 » — mois-année parlé, résolu en FIN de mois. */
const SPOKEN_MONTH = new RegExp(`garanti\\w*[^0-9]{0,24}\\b(${MONTH_ALTERNATION})\\s+(\\d{4})\\b`);
...
export function extractSpokenWarranty(message: string): SpokenWarranty {
  ...
  const numericSlash = /garanti\w*[^0-9]{0,24}(\d{2})\/(\d{2})\/(\d{4})/.exec(normalized);
  if (numericSlash) {
    return {
  ...
  const numericIso = /garanti\w*[^0-9]{0,24}(\d{4})-(\d{2})-(\d{2})/.exec(normalized);
  if (numericIso) {
    return {
