...
describe('replay', () => {
  it('summarizeRun compte les phases', async () => {
    const rt = new AgentRuntime({
      tools: [tool('documents_liste'), tool('factures_impayees')],
      clock,
      ids,
      executionAuthority,
    });
    const rec = await rt.run([inv('documents_liste'), inv('factures_impayees')]);
  ...
  it('dry-run -> invocationsFrom -> live rejoue exactement le plan', async () => {
    const a = tool('documents_liste');
    const b = tool('factures_impayees');
    const rt = new AgentRuntime({ tools: [a, b], clock, ids, executionAuthority });
    const dry = await rt.run(
      [inv('documents_liste'), inv('factures_impayees')],
      { mode: 'dry-run' },
    );
    expect(a.calls + b.calls).toBe(0);
    const replayInvocations = invocationsFrom(dry.entries);
    expect(replayInvocations.map((i) => i.tool)).toEqual([
      'documents_liste',
      'factures_impayees',
    ]);
  ...
  it('renderReplay produit des lignes lisibles (icône par phase)', async () => {
    const rt = new AgentRuntime({
      tools: [tool('documents_liste')],
      clock,
      ids,
      executionAuthority,
    });
    const rec = await rt.run([inv('documents_liste')]);
    const text = renderReplay(summarizeRun(rec));
    expect(text).toContain('✓ do documents_liste');
    expect(text).toContain('• do documents_liste');
  });
});
