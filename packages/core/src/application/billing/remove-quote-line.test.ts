...
import { RemoveQuoteLine } from './remove-quote-line';
...
function draftQuote(): Quote {
  ...
  q.addLine({ id: 'line-2', label: 'Fourniture', category: 'supply', qty: 1, unitPriceHT: 5000, vatRate: 10 });
  return q;
}

function makeRepo(quote: Quote | null): { quotes: QuoteRepository; saves: () => number } {
  const map = new Map<string, Quote>();
  if (quote) map.set(quote.id, quote);
  let saves = 0;
  const quotes: QuoteRepository = {
    findById: async (id) => map.get(id) ?? null,
    lockById: async (id) => map.get(id) ?? null,
  ...
  return { quotes, saves: () => saves };
}

describe('RemoveQuoteLine', () => {
  it('supprime une ligne du devis brouillon et persiste', async () => {
    const quote = draftQuote();
    const { quotes, saves } = makeRepo(quote);
    const r = await new RemoveQuoteLine({ quotes }).execute({ quoteId: quote.id, lineId: 'line-1' });

    expect(r).toEqual({ ok: true, value: { status: 'draft' } });
    expect(saves()).toBe(1);
    const saved = await quotes.findById(quote.id);
    expect(saved?.lines.map((l) => l.id)).toEqual(['line-2']);
  });

  it('quote introuvable -> not_found', async () => {
    const { quotes } = makeRepo(null);
    const r = await new RemoveQuoteLine({ quotes }).execute({ quoteId: 'missing', lineId: 'line-1' });
  ...
  it('ligne introuvable -> domain error, aucune sauvegarde', async () => {
    const quote = draftQuote();
    const { quotes, saves } = makeRepo(quote);
    const r = await new RemoveQuoteLine({ quotes }).execute({ quoteId: quote.id, lineId: 'missing' });
