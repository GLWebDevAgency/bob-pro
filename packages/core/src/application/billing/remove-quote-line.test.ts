...
describe('RemoveQuoteLine', () => {
  it('supprime une ligne du devis brouillon et persiste', async () => {
    ...
    const r = await new RemoveQuoteLine({ quotes, uow }).execute({ quoteId: quote.id, lineId: 'line-1' });
  ...
  it('quote introuvable -> not_found', async () => {
    ...
    const r = await new RemoveQuoteLine({ quotes, uow }).execute({ quoteId: 'missing', lineId: 'line-1' });
  ...
  it('ligne introuvable -> domain error, aucune sauvegarde', async () => {
    const quote = draftQuote();
    const { quotes, uow, counts } = makeRepo(quote);
    const r = await new RemoveQuoteLine({ quotes, uow }).execute({ quoteId: quote.id, lineId: 'missing' });
