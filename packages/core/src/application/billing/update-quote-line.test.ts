...
describe('UpdateQuoteLine', () => {
  it('modifie une ligne du devis brouillon et persiste', async () => {
    ...
    const r = await new UpdateQuoteLine({ quotes }).execute({
      quoteId: quote.id,
      lineId: 'line-1',
  ...
  it('quote introuvable -> not_found', async () => {
    ...
    const r = await new UpdateQuoteLine({ quotes }).execute({ quoteId: 'missing', lineId: 'line-1', patch: { qty: 2 } });
  ...
  it('ligne introuvable -> domain error, aucune sauvegarde', async () => {
    const quote = draftQuote();
    const { quotes, saves } = makeRepo(quote);
    const r = await new UpdateQuoteLine({ quotes }).execute({ quoteId: quote.id, lineId: 'missing', patch: { qty: 2 } });
  ...
  it('devis signé (hors draft) -> domain error INVALID_TRANSITION', async () => {
    ...
    quote.send(AT);
    const { quotes, saves } = makeRepo(quote);
    const r = await new UpdateQuoteLine({ quotes }).execute({ quoteId: quote.id, lineId: 'line-1', patch: { qty: 2 } });
    expect(r.ok).toBe(false);
