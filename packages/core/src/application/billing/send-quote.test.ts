...
function quoteDouble(overrides: Partial<{ id: string; companyId: string; number: string | null; status: string }> = {}): Quote {
  return {
...
describe('SendQuote — no-gap transactionnel', () => {
  it("n'alloue pas un deuxième numéro quand le devis est déjà envoyé", async () => {
    ...
    const r = await new SendQuote({ quotes, counters, uow, clock }).execute({ quoteId: 'quote-1' });
  ...
  it('rollback le compteur si save échoue après allocation', async () => {
    ...
    const send = new SendQuote({ quotes, counters, uow, clock });
