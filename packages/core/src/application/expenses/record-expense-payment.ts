...
export class RecordExpensePayment {
  ...
  async execute(
    ...
    const normalized = normalizeExpensePaymentEvidence(input, { today: this.deps.clock.today() });
    ...
    if (expense.status === 'paid') {
      if (!persistedEvidence)
        return err(appConflict(
          'expense_payment',
          'La dépense est déjà marquée payée sans preuve fiable ; une régularisation comptable est requise.',
        ));
    ...
    if (!alreadyRecorded) {
      const transition = expense.recordPayment(normalized.value, { today: this.deps.clock.today() });
